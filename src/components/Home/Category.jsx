...
export default function Category({ initialCategories = [] }) {
  ...
  return (
    <section
      className={`relative overflow-hidden bg-[#FAFAFA] pt-5 md:pt-2 pb-5 md:pb-4 ${sans.className}`}
    >
      {/* HEADING */}
      <div className="relative z-10 pt-4 md:pt-4 md:px-15 px-4 mb-4 md:mb-4 max-w-[1400px] mx-auto">
        ...
        <h2 className="text-3xl md:text-5xl lg:text-6xl font-black text-[#131313] leading-[1.05] mt-1 md:mt-2 tracking-tight">
          Trending{" "}
          <span className="bg-gradient-to-r from-[#8B5CF6] via-[#EC4899] to-[#F97316] bg-clip-text text-transparent font-extrabold pr-2">
            Category
          </span>
        </h2>
      </div>
            ...
            <div className="hidden sm:grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5 md:gap-6 lg:gap-8 max-w-[1400px] mx-auto">
              {categories.map((category, index) => (
                ...
                >
                  <motion.div
                    initial={
                      isMobile ? { opacity: 1, y: 0 } : { opacity: 0, y: 40 }
                    }
                  ...
                  >
                    ...
                    <div className="w-full px-4 py-4 bg-white">
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <h3
                            className={`text-[#131313] font-black capitalize tracking-tight group-hover:text-[#EC4899] transition-all duration-300 ${
                              category.name
                                .toLowerCase()
                                .includes("bar stools") ||
